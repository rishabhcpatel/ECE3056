...
*/
void compute_stats() {
    double acessTime = (((stats.reads + stats.writes) * MEMORY_READ_TIME) + (stats.page_faults * DISK_PAGE_READ_TIME) + stats.writebacks * DISK_PAGE_WRITE_TIME);
    stats.aat = acessTime/stats.accesses;
}
