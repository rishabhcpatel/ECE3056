...
pfn_t free_frame(void) {
    ...
    if (frame_table[victim_pfn].mapped ==  1 ) {
        //pfn_t victimPTBR = frame_table[victim_pfn].process->saved_ptbr;
        pte_t *page = (pte_t *)(mem+(frame_table[victim_pfn].process->saved_ptbr)*PAGE_SIZE) + frame_table[victim_pfn].vpn;
