...
void proc_init(pcb_t *proc) {
    ...
    pfn_t frameLoc = free_frame();

    memset(mem + (frameLoc*PAGE_SIZE),0,PAGE_SIZE);
    /*
     ...
     */
    proc->saved_ptbr = frameLoc;
    frame_table[frameLoc].protected = 1;
    frame_table[frameLoc].process = proc;
}
...
uint8_t mem_access(vaddr_t address, char rw, uint8_t data) {
    ...
    if (pageTable->valid == 0) {
        page_fault(address);
    }

    pfn_t presPFN = pageTable->pfn;
    /* Set the "referenced" bit to reduce the page's likelihood of eviction */
    frame_table[presPFN].referenced = 1;
   ...
   physicalAddress = (paddr_t)(presPFN*PAGE_SIZE) + offset;
...
void proc_cleanup(pcb_t *proc) {
    /* Look up the process's page table */
      pte_t* pageTableE = (pte_t*) (mem + ((proc->saved_ptbr) * PAGE_SIZE));

    /* Iterate the page table and clean up each valid page */
    for (size_t i = 0; i < NUM_PAGES; i++) {
        
        if ((pageTableE + i)-> valid == 1) {
            (frame_table + (pageTableE + i)->pfn)->mapped = 0;
            (frame_table + (pageTableE + i)->pfn)->protected = 0;
        }        

        if (swap_exists(pageTableE + i) == 1) {
            swap_free(pageTableE + i);
        }

        (pageTableE + i)->valid = 0;
        (pageTableE + i)->dirty = 0;
    }
